...
Alongside the flagship paper, four focused companion papers (an honest, disclosed split of the same project, each with a distinct contribution; they cross-cite) are in [`papers/`](papers/), each with its PDF, sources, figures, and deterministic experiments under [`papers/experiments/`](papers/experiments/):
...
- **P4 — [Algorithmic Bias Against Arabic](papers/arabic_bias/)**: a measurement, and three frozen-model mitigations, in an offline multilingual retrieval system. The honest finding from a clean matched-pipeline test (n=390): the multilingual relevant-score geometry differs significantly by language but in **no stable direction** — it flips with corpus homogeneity, genre, and model, so neither "Arabic compressed" nor "Arabic scores lower" is a transferable law; the one stable disadvantage is ~1.3× tokenization fertility. This instability is precisely why the cutoff must be **measured per-tenant**. Scientific (48 pp, 12 figures) + plain-language (101 pp) versions.
