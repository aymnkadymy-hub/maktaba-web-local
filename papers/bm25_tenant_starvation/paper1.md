...
We describe **maktaba-web-local** (al-Maktaba al-Natiqa, "The Speaking Library"), a fully-offline, on-device, multi-tenant Arabic+English digital-library RAG system, and within it the diagnosis and remedy of sparse-retrieval tenant starvation.
...
**Dominance control.** To vary `δ` we deterministically sub-sample tenant A to sizes giving `δ ∈ {0.50, 0.80, 0.90, 0.95, 0.98, 0.985}` (the last being the full real corpus), holding tenant B fixed, and average over five fixed seeds (0–4).
...
![Figure 2: Oracle-overlap@5 collapses for shared-vocabulary minority queries as dominance rises (red), but is flat for the distinct-vocabulary control (blue); the per-tenant sub-index is exact (green).](figures/fig1_overlap_vs_dominance.png)

![Figure 3: The minority candidate pool (yield of k=5) depletes with dominance for shared-vocabulary queries.](figures/fig2_yield_vs_dominance.png)
...
![Figure 4: Over-fetch raises target recall (and removes zero-yield) but does not restore the candidate-pool depletion of Figure 3.](figures/fig3_recall_vs_overfetch.png)
...
We co-measure the dense path on the identical corpus, probes, and metric, embedding every chunk with the system's `paraphrase-multilingual-MiniLM-L12-v2` model and searching an *exact* FAISS inner-product index (rather than the deployed approximate FAISS-HNSW, so that the measured collapse is attributable to crowding alone and not to ANN approximation error) with the deployed over-fetch-then-post-filter (`fetch_k = 8k`), averaged over five fixed seeds (0–4).
...
![Figure 5: Measured: crowding starves *both* sparse (BM25, 18× over-fetch) and dense (FAISS, 8× over-fetch) post-filtering as tenant dominance rises; the per-tenant sub-index restores both to overlap 1.0.](figures/fig4_sparse_vs_dense.png)
...
- **Passage-derived probe queries.** The probe target is a span taken from a B passage, which makes that passage an easy self-match and is the reason target recall@5 stays 1.0 at the deployed over-fetch even under heavy starvation. This *understates* the failure for natural questions, where no privileged self-match exists and the relevant passages are ordinary B passages competing on shared vocabulary — exactly the passages that Table 1 shows being depleted. We report yield and oracle-overlap precisely because they do not depend on the privileged target, and we flag the absence of a natural-question log as the single threat most likely to change the *magnitude* (though not the direction) of our numbers.
...
[6] Hu, M., et al. (2024). No free lunch: Retrieval-augmented generation undermines fairness in LLMs, even for vigilant users. arXiv:2410.07589
